...
def test_unicode_char(filename):
    ...
    pattern = re.compile("[\u200B-\u200E\uFEFF\u202c\u202D\u2063\u2062]") #zero width characters
    for i, line in enumerate(open(filename)):
        for match in re.finditer(pattern, line):
            logger.debug('Unicode char in FILE %s Line %s: %s' % (filename, i+1, match.group().encode('utf-8')))
            INVALID = True
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i','--input', help='Input-file/folder to test unicode chars', required=True)
