    ...
    for event in root.iter("event"):
        ...
        if transport_type in transport_type_list:
            ...
            if not (host and source and source_type):
                logger.error(
                    f'ERROR: Filename:{filename} Event_no:{event_no} with transport_type:{transport_type} has missing value.\nhost:"{host}" source:"{source}" or sourcetype:"{source_type}"'
                )
                global INVALID
                INVALID = True
        event_no += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input", help="Input-file/folder to test tranport params", required=True
    )
